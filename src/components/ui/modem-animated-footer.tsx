...
export const Footer = ({
    ...
    return (
        <section className={cn("relative w-full mt-0 overflow-hidden", className)}>
            <footer className="border-t border-white/10 bg-black mt-20 relative text-white">
                            ...
                            {navLinks.length > 0 && (
                                <div className="flex flex-wrap justify-center gap-4 text-sm font-medium text-gray-400 max-w-full px-4">
                                    {navLinks.map((link, index) => (
                                        <a
                                            key={index}
                                            className="hover:text-white duration-300 hover:font-semibold cursor-pointer"
                                            href={link.href}
                                        >
                                            {link.label}
                                        </a>
                ...
                {/* Bottom logo */}
                <div className="absolute hover:border-white duration-400 drop-shadow-[0_0px_20px_rgba(255,255,255,0.1)] bottom-24 md:bottom-20 backdrop-blur-sm rounded-3xl bg-black/60 left-1/2 border-2 border-white/20 flex items-center justify-center p-3 -translate-x-1/2 z-10">
                    <div className="w-12 sm:w-16 md:w-24 h-12 sm:h-16 md:h-24 flex items-center justify-center relative overflow-hidden">
                        {brandIcon}
                    </div>
                </div>
