                    ...
                    >
                        <CarouselContent className="ml-0">
                            {logos.map((logo) => (
                                <CarouselItem
                                    key={logo.id}
                                    className="flex basis-1/2 sm:basis-1/3 md:basis-1/5 lg:basis-1/6 justify-center pl-0"
                                >
                                    <div className="mx-4 flex shrink-0 items-center justify-center bg-white/5 p-4 rounded-xl backdrop-blur-sm border border-white/10 w-full h-24">
                                        <div>
                                            <img
                                                src={logo.image}
                                                alt={logo.description}
                                                className={logo.className + " brightness-0 invert opacity-70 hover:opacity-100 transition-opacity duration-300"}
                                            />
                                        </div>
                                    </div>
                                </CarouselItem>
